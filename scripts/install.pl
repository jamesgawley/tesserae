...
directories. Under normal circumstances, it should not be necessary to
move the various subfolders of the root Tesserae directory, in in this
case the present script won't have to move anything.

This script also generates defs.php, which is necessary for the web
interface, and calls F<scripts/doc-gen.pl>, which creates the online
documentation.
