...
=item --lang I<LANG> [--lang I<LANG> ...]

Languages to set up initially. Tesserae expects I<LANG> to correspond
to a subdirectory of F<texts/>, containing all the works in that
language. The version of Tesserae on GitHub has a large number of
texts in Greek (F<texts/grc>) and Latin (F<texts/la>), mostly from 
Perseus, as well as a couple of experimental texts in English 
(F<texts/en>), of diverse provenance. You can use the flag more
than once to select multiple languages. The default setting is 
C<--lang la --lang grc>; i.e., if you don't specify a language
...
=item --feature I<FEAT> [--feature I<FEAT>]
...
Use F<scripts/v3/clean.pl> to delete existing texts and feature
dictionaries before installing texts. This will run B<clean.pl>
with the flags C<--text --dict LANG [LANG ...]>, for each of the
...
Lingua::Stem, by running add_col_stem.pl with the B<--use-lingua> flag.
So if you run this script with the option C<--lang en>, turn off stem
indexing by specifying C<--feat ""> (i.e. nothing inside double-quotes) 
...
do_cmd(catfile($fs{script}, 'build-stem-cache.pl '. join(" ", @inst_lang)));
do_cmd(catfile($fs{script}, 'patch-stem-cache.pl'));
